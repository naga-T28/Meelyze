...
- 色、アイコン、完全な状態ラベルの3要素を必ず同時に表示し、色またはアイコンだけで状態を表さない。**例外**: S08の画像上重畳タグ（原文の位置・大きさに直接載せる最小サイズのタグ、FIX-015）に限り、完全な状態ラベルの文字列は省略してよい。この場合も色とアイコンは必ず併用し、省略した完全ラベル・判明済み対象食材・判定不可理由はタップ後のS09、および`accessibilityLabel`（VoiceOver）で必ず提供する。単なる色の濃淡・彩度だけの違いに置き換えることは認めない。
...
S08のBounding Box overlay自体は画像内の位置を保つため、危険度順に並べ替えない。別途表示する結果summary/listとVoiceOverの走査順は、`含有の可能性が高い`、`判定不可`、`収録データ上は該当なし`の順とし、同じ状態内は画面上の読み順を維持する。S09では料理名の日本語原文と選択言語訳を併記する。S08の画像上重畳タグは原文と同じ大きさ・位置に収める最小サイズのため、翻訳済み名称（翻訳不可時は日本語原文）のみを表示し、日本語原文と選択言語訳の併記はタップ後のS09で行う（FIX-015、Google翻訳のカメラ翻訳のような表示を意図したユーザー判断）。Translation利用不可時は日本語原文と三値を残し、翻訳不可をE04として示すだけで判定順・三値・Evidenceを変更しない。
