...
/// 特定原材料8品目・特定原材料に準ずるもの20品目（計28品目）のカタログ。
///
/// 出典: 消費者庁「食物アレルギー表示に関する情報」（くるみの義務化を反映した2023年改正後の品目）。
/// 品目は法令改正により変わり得るため、実装着手時に消費者庁の最新公表資料で再確認すること
...
enum AllergenItem: String, CaseIterable, Identifiable, Hashable, Sendable {
    ...
    /// 特定原材料（表示義務8品目）であれば`true`、特定原材料に準ずるもの（表示推奨20品目）であれば`false`。
    ...
    private var localizedNames: LocalizedAllergenNames {
        ...
        case .matsutakeMushroom:
            return LocalizedAllergenNames(japanese: "まつたけ", english: "Matsutake mushroom", traditionalChinese: "松茸", simplifiedChinese: "松茸", korean: "송이버섯")
        case .peach:
